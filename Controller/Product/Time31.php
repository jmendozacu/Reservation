 ...
 */
class Time31 extends \Magento\Framework\App\Action\Action
{
    protected $_logger;
    ...
    public function execute()
    {
        ...
        $schedules = $this->_productScheduleWithoutStaffFactory->create()->getCollection()->addFieldToFilter('product_id', $productId);
        ...
        foreach ($schedules as $schedule) {
            ...
            if ($oldOrders) {
                foreach ($oldOrders as $oldOrder) {
                    if ($oldOrder[0] == $date && $schedule->getWeekday() == $dayFilter && $oldOrder[1] == $schedule->getSlots()) {
                        $orderAdded = 1;
                        break;
                    } elseif ($oldOrder[0] == $date && $schedule->getWeekday() == $dayFilter && $oldOrder[1] < $schedule->getSlots()) {
                ...
                if ($schedule->getWeekday() == $dayFilter) {
                    ...
                    if ($specialDateModel) {
                        foreach ($specialDateModel as $specialDate) {
                            ...
                            if (strcmp($newDate . ' ' . ' 00:00:00', $fromTemp) >= 0 && strcmp($newDate . ' ' . ' 23:59:59', $toTemp) <= 0) {
                        ...
                        foreach ($reservationRuleData as $reservationRuleDataItem) {
                            ...
                            switch ($option) {
                                ...
                                case 4:
                                    ...
                                    if (strcmp(date_format($newDateNum, 'm/d') . ' ' . $schedule->getFromTime() . ":00", $fromDate) >= 0
                                        && strcmp(date_format($newDateNum, 'm/d') . ' ' . $schedule->getToTime() . ":00", $toDate) <= 0
                                    ) {
                                        if ($reservationRuleAdded == 0) {
                                            $reservationRuleAdded = 1;
                                            $specialName .= __(', Rush hour');
